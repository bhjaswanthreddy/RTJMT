...
useEffect(() => {
  ...
  <section className="max-w-3xl mx-auto bg-[#2a2a3c] p-8 rounded-xl shadow-md">
    ...
    <p className="text-gray-300 mb-4">Upload your resume to find the most relevant job listings based on your skills.</p>
    ...
    <input
      ...
      onChange={async (e) => {
        ...
        const jobs = jobData.filter(job =>
          skills.every(skill => job.Skills?.toLowerCase().includes(skill.toLowerCase()))
        );
        setResumeJobs(jobs);
        setCurrentPage(1);
      }}
    ...
    <p className="text-sm text-gray-400 mb-4">Accepted formats: .txt, .pdf (text-only PDFs)</p>

    {matchedSkills.length > 0 && (
      <>
        <p className="text-white font-semibold mb-2">Matched Skills:</p>
        <div className="text-sm text-gray-300 mb-4">{matchedSkills.join(', ')}</div>
        <p className="text-white font-semibold mb-4">Matched Jobs: {resumeJobs.length}</p>

        {/* Paginated Jobs */}
        <div className="space-y-4">
          {resumeJobs.slice((currentPage - 1) * 15, currentPage * 15).map((job, idx) => (
            <div key={idx} className="bg-[#1e1e2f] p-4 rounded-lg border border-gray-700">
              <p className="font-bold text-white">{job.Job_Title}</p>
              <p className="text-sm text-gray-400">{job.Company} • {job.Location}</p>
              <p className="text-sm text-gray-400">Skills: {job.Skills}</p>
            </div>
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          <span className="text-gray-400 text-sm">
            Page {currentPage} of {Math.ceil(resumeJobs.length / 15)}
          </span>
          <button
            onClick={() => setCurrentPage(p => Math.min(Math.ceil(resumeJobs.length / 15), p + 1))}
            disabled={currentPage >= Math.ceil(resumeJobs.length / 15)}
            className="px-4 py-2 rounded bg-gray-700 text-white hover:bg-gray-600 disabled:opacity-50"
          >
            Next
          </button>
